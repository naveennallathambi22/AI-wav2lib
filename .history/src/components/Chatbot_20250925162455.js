...
const Chatbot = ({ 
  ...
  speak(formattedAnswer, selectedLanguage);
      setIsTyping(false);
  ...
  const speak = async (text, lang = selectedLanguage) => {
    if (!text) return;

    setIsTyping(true); // Show typing indicator

    const media = await generateLipSyncVideo(text, lang);
