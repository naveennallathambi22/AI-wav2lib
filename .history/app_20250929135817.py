...
# Cleanup function to delete video and audio after a delay
...
# Load ONLY English Indian voice
...
def generate_video():
    ...
    # lang = data.get('lang', 'en')  # Optional: keep for future, but unused

    uid = str(uuid.uuid4())
    audio_path = f"{OUTPUT_DIR}/{uid}.wav"
    video_path = f"{OUTPUT_DIR}/{uid}.mp4"

    # Always use English Indian voice (works for Hindi/Marathi text too)
